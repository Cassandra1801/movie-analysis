...
public class MovieService {
    ...
    public Movie getLast() {
        return allMovies.get(allMovies.size() - 1);
    }
    ...
    public List<Movie> getAdventureMovies() {
        List<Movie> adventureMovies = new ArrayList<>();
    ...
    public double getAverageAdventureLength() {
        List<Movie> adventureMovies = getAdventureMovies();

        if (adventureMovies.isEmpty()) {
            return 0;
        }

        return getAverageLength(adventureMovies);
    }

    public List<Movie> getFirstTenAwardMovies() {
        List<Movie> firstTenAwardMovies = new ArrayList<>();

        for (Movie movie : allMovies) {
            if (movie.isAwards()) {
                firstTenAwardMovies.add(movie);
            }

            if (firstTenAwardMovies.size() == 10) {
                break;
            }
        }

        return firstTenAwardMovies;
    }
}
